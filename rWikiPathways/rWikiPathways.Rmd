...
Finally, there's another way to find pathways of interest: by Xref. An Xref is simply a standardized identifier form an official source. WikiPathways relies on BridgeDb to provide dosens of Xref sources for genes, proteins and metabolites.  
...
```{r}
getXrefList(pathway = "WP554", systemCode = "En")
```
...
HMDB  
```{r}
getXrefList(pathway = "WP554", systemCode = "Ch")
```

ChEBI  
```{r}
getXrefList(pathway = "WP554", systemCode = "Ce")
```

Chemspider  
```{r}
getXrefList(pathway = "WP554", systemCode = "Cs")
```
...
```{r}
gpml <- getPathway(pathway = "WP554", revision = 0)
```
...
```{r}
getXrefList(pathway = "WP554", systemCode = getSystemCode(name = "Ensembl"))
```

3. Getting all the metabolites from a pathway as ChEBI  

```{r}
getXrefList(pathway = "WP554", systemCode = getSystemCode(name = "ChEBI"))
```
...
```{r}
wp2gene <- clusterProfiler::read.gmt(gmtfile = wp.hs.gmt)
