...
- for multiple frequently studied model organisms, usch as mouse, rat, pig, zebrafish, fly, and yeast in addition to the original human genes.  
...
retrive data for a specific collection, such as the hallmark gene sets  
...
clusterProfiler::enricher(gene = gene_ids_vector, TERM2GENE = msigdb_t2g, ...)
```
...
clusterProfiler::enricher(gene = gene_symbols_vector, TERM2GENE = msigdbr_t2g, ...)
```
...
```
msigdbr_list <- split(msigdbr_df$gene_symbol, f = msigdbr_df$gs_name)
