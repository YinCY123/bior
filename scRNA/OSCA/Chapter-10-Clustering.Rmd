...
sce.pbmc <- readRDS("../../../../Data/Bioconductor/osca/sce.pbmc.rds")
...
```{r}
pure.pbmc <- neighborPurity(x = reducedDim(sce.pbmc, "PCA"), clusters = clust)

pure.data <- as.data.frame(pure.data)
pure.data$maximum <- factor(pure.data$maximum)
pure.data$cluster <- factor(clust)

ggplot(pure.data, aes(cluster, purity, colour = maxium)) +
    ggbeeswarm::geom_quasirandom(method = "smiley")
```
...
pheatmap(tab, color = viridis(100), cluster_cols = F, cluster_rows = F)
```

For clusterings that differ primarily in resolution (usually from different parameterizations of the same algorithm), we can use the clustree package to visualize the relationships between them. Here, the aim is to capture the redistribution of cells from one clustering to another at progressively higher resolution, providing a convenient depiction of how clusters split apart. This approach is most effective when the clusterings exhibit a clear gradation in resolution but is less useful for comparisons involving theoretically distinct clustering procedures.  

```{r}
library(clustree)

combined <- cbind(k.50 = clust.50, k.10 = clust, k.5 = clust.5)
clustree(combined, prefix = "k.", edge_arrow  =F)
```
...
```{r}
set.seed(0010010100)
ratios <- bootstrapStability(x = pcs, FUN = myClusterFUN, clusters = originals)
ratios %>% dim
```

The function returns a matrix of ARI-derived ratios for every pair of original clusters in · (Figure 10.20), averaged across bootstrap iterations. High ratios indicate that the clustering in the bootstrap replicates are highly consistent with that of the original dataset. More specifically, high ratios on the diagonal indicate that cells in the same original cluster are still together in the bootstrap replicates, while high ratios off the diagonal indicate that cells in the corresponding cluster pair are still separted.  

```{r}
pheatmap(ratios, 
         cluster_rows = F, 
         cluster_cols = F, 
         color = viridis(100, oprion = "B"), 
...
clust.full <- igraph::cluster_walktrap(graph = g.full)$membership
...
```{r}
# repeating modeling and PCA on the subset
memory <- 10L
sce.momory <- sce.pbmc[, clust.full == memory]
dec.momory <- modelGeneVar(sce.momory)
sce.momory <- denoisePCA(sce.momory, technical = dec.momory, 
                         subset.row = getTopHVGs(dec.momory, n = 5000))
```
...
```{r}
g.memory <- buildSNNGraph(sce.momory, use.dimred = "PCA")
clust.memory <- igraph::cluster_walktrap(g.memory)$membership

plotExpression(sce.momory, 
               features = c("CD8A", "CD4"), 
...
```{r}
set.seed(100010)
subcluster.out <- quickSubCluster(x = sce.pbmc, groups = clust.full, 
                                  prepFUN = function(x){
                                      dec = modelGeneVar(x)
...
subcluster.out %>% names
```

```{r}
subcluster.out[[1]]$subcluster %>% unique() %>% sort
```
