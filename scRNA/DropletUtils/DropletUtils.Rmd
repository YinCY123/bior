...
`CellRanger` will also produce a molecule information file (molecule_info.h5) that contains… well, information about the transcript molecules. This includes the UMI sequence, the cell barcode sequence, the gene to which it was assigned, and the number of reads covering the molecule.  
...
We recommend making some diagnostic plots such as the total count againt the negative log-probability. Droplets detected as cells should show up with large negative log-probability or very large total counts (based on the knee point reported by `barcodeRanks`). Note that the example below is based on simulated data and is quite exaggerated.
...
We also identify doublets based on the log-flod change between the second HTO's abundance and the ambient contamination. A large log-fold change indicates that the second HTO exceeds that from contamination, consistent with the presence of a doublet.
