...
?randomForest()
logmtx <- logcounts(GSE122465) %>% as.data.frame()
logmtx %>% tibble::rownames_to_column("ensembl") %>% 
    tidyr::pivot_longer(cols = -ensembl, names_to = "barcodes", values_to = "logcounts")
logmtx <- .Last.value
cell_meta <- colData(GSE122465) %>% as.data.frame()
cell_type <- setNames(cell_meta$cell_types, cell_meta$barcodes)

logmtx$cell_type <- cell_type[logmtx$barcodes]
logmtx$logcounts %>% range()

classifier <- randomForest(cell_type ~., data = logmtx, importance = TRUE, ntree = 500)
```
