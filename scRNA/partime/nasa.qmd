...
go_res %>% lapply(function(x){print(x)})
```
...
for(con in names(go_res)){
    for(cell in to_go){
        ...
        ggsave(paste("/home/yincy/git/data/partime/nasa/res/GO/", con, "_", cell, ".tiff", sep = ""), 
               width = 12)
    }
}
```
...
for(con in names(kegg_res)){
    write.xlsx(kegg_res[[con]], file = paste("/home/yincy/git/data/partime/nasa/res/GO/", con, "_kegg.xlsx", sep = ""))
}
```
