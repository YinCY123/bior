...
fmarkers <- findMarkers(x = nasa, 
                        groups = nasa$levels, 
                        restrict = paste(c("FLT", "GC"), contr[1], sep = "_"), 
...
```{r}
contr <- gsub("^FLT_|GC_", "", unique(nasa$levels)) %>% unique
degs <- vector(mode = "list")

for(i in seq_along(contr)){
    cur <- findMarkers(nasa, 
                       groups = nasa$levels, 
                       restrict = paste(c("FLT", "GC"), contr[i], sep = "_"), 
                       assay.type = "logcounts", 
                       pval.type = "all")
...
markers %>% length
markers %>% names
names(markers) <- paste("FLT", names(markers), sep = "_")
saveRDS(markers, "../../../data/partime/nasa/all_degs.rds")
```
...
all_degs <- readRDS("../../../data/partime/nasa/all_degs.rds")

immune_cells <- c("large pre-B.", "Dendritic cells", "Monocytes", 
                  "Neutro prog.", "small pre-B.", "B cell", 
                  "Neutrophils", "NK cells", "Mono prog.", 
                  "T cells")
...
# binary matrix
```{r}
union_genes <- Reduce(union, lapply(all_degs, rownames))
union_genes %>% str
...
```{r}
all_degs_old <- all_degs[1:21]
all_degs_yng <- all_degs[22:42]
```

```{r}
names(all_degs_old) <- str_extract_after(names(all_degs_old), pattern = "_", which = "last")
all_degs_old %>% names
```

```{r}
for(cell in names(all_degs_old)){
    up <- all_degs_old[[cell]] %>% as.data.frame() %>%  dplyr::filter(FDR < 0.05, summary.logFC > 0) %>% rownames()
    down <- all_degs_old[[cell]] %>% as.data.frame() %>% dplyr::filter(FDR < 0.05, summary.logFC < 0) %>% rownames()
    # nc <- setdiff(rownames(all_degs_old[[cell]]), unique(c(up, down)))
    mtx_old[up, cell] <- 1
    mtx_old[down, cell] <- -1
}
...
pheatmap(mtx_old, 
         ...
         color = colorRampPalette(colors = c("blue", "grey90", "red"))(3))
```
