...
nasa_agg <- aggregateAcrossCells(nasa, 
                                 ids = nasa$clusters,
                                 use.assay.type = "logcounts",
                                 BPPARAM = BiocParallel::MulticoreParam(10))
GSE122465_agg <- aggregateAcrossCells(GSE122465, 
                                      ids = GSE122465$cell_types,
                                      use.assay.type = "logcounts",
                                      BPPARAM = BiocParallel::MulticoreParam(10))
GSE122465$cell_types %>% table %>% length
GSE122465_agg$cell_types %>% table %>% length

pred_label <- SingleR(test = nasa_agg, 
                      ref = GSE122465_agg, 
                      labels = GSE122465_agg$cell_types, 
                      aggr.ref = F, 
                      de.method = "t",
                      de.n = 100, 
...
nasa_agg$cell_type <- pred_label$pruned.labels
nasa_agg %>% colData()
cell_types <- setNames(nasa_agg$cell_type, nasa_agg$clusters)
TRUE
...
ggsave("/home/yincy/git/data/parttime/nasa/nasa.cell_type.tiff")
```
