...
We can use self-organizing maps (SOMs) from the `kohonen` package. This allocates observations to nodes of a simple neural network; wach node is treated as a cluster.  
...
We can shared or direct nearest neighor graphs and perform community detection with `igraph`. Here, the number of neighors `k` controls the resolution of the clusters.  
