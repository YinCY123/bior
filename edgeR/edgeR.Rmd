...
- glm `edgeR`: multifactor experiment  
...
the UTR as part of the first exon.  
...
The function accesses the group factor contained in y in order to compute the minimum group size, but the filtering is performed independently of which sample belongs to which group so that no bias is
introduced.  
...
```{r}
y <- read.csv(file = "GSE131998/GSE131998_HPASMC_EV_Cells_Raw_ReadCount.csv", header = T)

counts <- y[, colnames(y)[8:23]]
genes <- y[, colnames(y)[1:7]]

rownames(counts) <- genes$ENSEMBL

pdata <- getGEO(GEO = 'GSE131998')
sample <- pData(pdata[[1]])
group <- paste(sample$`rna source (evs/cells):ch1`,sample$`treatment:ch1`, sep = "_")

dgelist <- DGEList(counts = counts, genes = genes, group = group)
```


```{r}
keep <- filterByExpr(y = dgelist, min.count = 1)
