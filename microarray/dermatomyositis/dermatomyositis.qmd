...
pdf("../../../data/zhuyi/dermatomyositis/res/02_feature_gene_screen/svm_feature_selection.pdf", width = 10)
plot(errors, 
     type = "b", 
...
genes %>% head
all_cohorts_list <- vector(mode = "list")
gene_list <- split(genes$symbol, f = cut(seq_along(genes$symbol), breaks = ceiling(length(genes$symbol)/200)))
```

```{r}
for(cohort in cohorts){
    if(cohort %in% names(all_cohorts_list)){
