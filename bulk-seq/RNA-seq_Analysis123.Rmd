...
# Abstract
The ability to easily and efficiently analyse RNA-sequencing data is a key strength of the Bioconductor project. Starting with counts summarised at the gene-level, a typical analysis involves pre-processing, exploraroty analysis, differential expression testing and pathway analysis with the results obtained informing future experiments and validation studies. In this workflow article, we analysis RNA-sequencing data from mouse mamary gland, demonstrating use of the popular **edgeR** package to import, filter and normalise the data, followed by the **limma** package with its *voom* method, linear modeling and empirical Bayes enhanced by the **Glimma** package which enables interactive exploration of the results so that individual samples and genes can be examined by the user. The complete analysis offered by these three packages highlights the ease with which researchers can turn the raw counts from an RNA-sequencing experiment into biological insights using Bioconductor.
...
To get started with this analysis, download the file GSE63310_RAW.tar available online, and extract the relevant files from this archive.

```{r}
getGEOSuppFiles(GEO = "GSE63310", baseDir = "../../data/GEO", makeDirectory = TRUE)
untar("../../data/GEO/GSE63310/GSE63310_RAW.tar", exdir = "../../data/GEO/GSE63310/")


files <- dir_ls("../../data/GEO/GSE63310/", regexp = "*.txt.gz$")
files
for(i in files){
    gunzip(i, overwrite = TRUE)
}

samples <- c("GSM1545535_10_6_5_11.txt", 
...
read.delim(paste("../../data/GEO/GSE63310/", samples[[1]], sep = ""), nrows = 5)
```
...
```{r}
x <- readDGE(files = paste("../../data/GEO/GSE63310/", samples, sep = ""), 
             columns = c(1, 3))
x %>% class()
x %>% dim
x$samples
...
samplenames <- str_extract_after(colnames(x), pattern = "_")
colnames(x) <- samplenames

x$samples$group <- factor(c("LP", "ML", "Basal", "Basal", "ML", "LP", "Basal", "ML", "LP"))
x$samples$lane <- factor(rep(c("L004", "L006", "L008"), c(3, 4, 2)))
x$samples
```
...
cols <- RColorBrewer::brewer.pal(n = 9, name = "Paired")

par(mfrow = c(1, 2))
plot(density(lcpm[, 1]), col = cols[1], lwd = 2, main = "", xlab = "", ylim = c(0, 0.26))
title("A. Raw data")
for(i in 2:dim(lcpm)[2]){
    den <- density(lcpm[, i])
    lines(den$x, den$y, col = cols[i], lwd = 2)
}
legend("topright", legend = samplenames, text.col = cols, bty = "n")


xlcpm <- cpm(x, log = TRUE)
plot(density(xlcpm[, 1]), col = cols[1], lwd = 2, main = "", xlab = "", ylim = c(0, 0.26))
title("B. Filtered data")
for(i in 2:dim(x)[2]){
    den <- density(xlcpm[, i])
    lines(den$x, den$y, col = cols[i], lwd = 2)
}
...
boxplot(lcpm, las = 2, col = cols, main = "")
...
colnames(design) <- gsub("x\\$samples\\$group|x\\$samples\\$lane", "", colnames(design))
```
...
## Removing heteroscedacity from count data
...
plotSA(efit, main = "Final model: mean-variance tred")
```
