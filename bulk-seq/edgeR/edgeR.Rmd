...
GDEList components:  
    ...
    - `lib.size`: for the library size or sequencing depth for each sample. If not specified it will be computed from the column sums of the counts.  
...
The default method for computing these scale factors uses a trimmed mean of Mvalues (TMM) between each pair of samples. We call the product of the *original library size* and the scaling factor the *effective library size*. The *effective library size* replaces the original library size in all downstream analyses.    
