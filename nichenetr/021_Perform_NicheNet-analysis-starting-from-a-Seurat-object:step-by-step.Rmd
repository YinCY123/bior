...
weighted_network_lr <- weighted_network_lr %>% mutate(from = convert_human_to_mouse_symbols(from), 
                                                      to = convert_human_to_mouse_symbols(to)) %>% 
    drop_na()
```
...
order_receptors <- order_recepotors %>% intersect(rownames(lr_network_top_matrix))
order_ligands <- order_ligands %>% intersect(colnames(lr_network_top_matrix))

vis_ligand_receptor_network <- lr_network_top_matrix[order_recepotors, order_ligands]
...
DE_table_all <- Idents(seuratObj) %>% 
    ...
    lapply(get_lfc_celltype, seurat_obj = seuratObj)
```
