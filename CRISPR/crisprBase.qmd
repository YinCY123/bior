...
```{r}
SpCas9 <- CrisprNuclease(nucleaseName = "SpCas9", 
                         targetType = "DNA", 
                         pams = c("(3/3)NGG", "(3/3)NAG", "(3/3)NGA"), 
                         weights = c(1, 0.2593, 0.0694), 
                         metadata = list(description = "wildtype streptococcus pyogenes Cas9 (SpCas9) nucleases"), 
                         pam_side = "3prime", 
                         spacer_length = 20)
SpCas9
```
...
```{r}
SaCas9 <- CrisprNuclease(nucleaseName = "SaCas9", 
                         targetType = "DNA", 
                         pams = c("(3/3)NNGRRT"), 
                         metadata = list(description = "Wildtype Staphylococcus aureus Cas9 (SaCas9) nucleases"), 
                         pam_side = "3prime", 
                         spacer_length = 21)
SaCas9
```
...
```{r}
AsCas12a <- CrisprNuclease(nucleaseName = "AsCas12a", 
                           targetType = "DNA", 
                           pams = "TTTV(18/23)", 
                           metadata = list(description = "Wildtype Acidaminococcus Cas12a (AsCas12a) nucleases."), 
                           pam_side = "5prime", 
                           spacer_length = 23)
AsCas12a
```
...
- `pam_site`: genomic coordinate of the first nucleotide of the nuclease-specif PAM sequence. For SpCas9, this corresponds to the genomic coordinate of N in the NGG PAM sequence. For AsCas12a, this corresponds to the genomic coordinbate of the first  nucleotide in the TTTV PAM sequence. For RNA-targeting nuclease, this corresponds to the first nucleotide of the PFS (we do not use `pfs_site` for simplicity).

![**CRISPR nuclease.**](figures/CRISPR_gRNA.png)
...
SpCas9 %>% cutSites(strand = "+")

AsCas12a %>% cutSites(strand = "+")
```


Below is an illustration of how different sequences and cut patterns translate into cut site coordinates with respect to a PAM sequence NGG.

![**cut site coordinates.**](figures/CRISPR_cut_site.png)
...
extractPamFromTarget(targets = targets, object = SpCas9)
```

```{r}
extractProtospacerFromTarget(targets = targets, object = SpCas9)
```
...
![**base editors**](figures/CRISPR_baseeditor.png)

We provide in `crisprBase` a S4 class, BaseEditor, to represent base editors. It extends the `CrisprNuclease` class with 3 additional fields.  
...
weightsFile <- system.file("be/b4max.csv", 
                           package = "crisprBase", 
                           mustWork = TRUE)
ws <- read.csv(weightsFile, row.names = 1) %>% t %>% as.data.frame()
ws
```
...
BE4max <- BaseEditor(CrisprNuclease = SpCas9, 
...
For instance, the D10A mutation of SpCas9 inactivates the RuvC domain, and the resulting CRISPR nickase (Cas9D10A) cleaves only the strand opposite to the protospacer sequence. The H840A mutation of SpCas9 inactivates the HNN domain, and the resulting CRISPR nickase (Cas9H840A) cleaves only the strand that contains the protospacer sequence.

![**CRISPR Nickase**](figures/CRISPR_nickease.png)

The `CrisprNickase` class in `crisprBase` works similar to the `CrisprNuclease` class:

```{r}
Cas9D10A <- CrisprNickase(nickaseName = "Cas9D10A", 
                          nickingStrand = "opposite", 
                          pams = c("(3)NGG", "(3)NAG", "(3)NGA"), 
                          weights = c(1, 0.2593, 0.0694), 
                          metadata = list(description = "D10A-mutated Streptococcus pyogenes Cas9 (SpCas9) nickase"), 
                          pam_side = "3prime", 
                          spacer_length = 20)

Cas9H840A <- CrisprNickase(nickaseName = "Cas9H840A", 
                           nickingStrand = "original", 
                           pams = c("(3)NGG", "(3)NAG", "(3)NGA"), 
                           weights = c(1, 0.2593, 0.0694), 
                           metadata = list(description = "H840A-mutated Streptococcus pyogenes Cas9 (SpCas9) nickase"), 
                           pam_side = "3prime", 
                           spacer_length = 20)
```

The `nickingStrand` field indicates which strand is being cleaved by the nickase.
...
```{r}
CasRx <- CrisprNuclease(nucleaseName = "CasRx", 
                        targetType = "RNA", 
                        pams = "N", 
