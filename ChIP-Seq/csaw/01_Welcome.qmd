...
# Quick Start
A typical `ChIP-seq` analysis in `csaw` would look something like that described below. This assumes that a vector of file paths to sorted and indexed BAM files is provided and a design matrix in suppied in. The code is split across several steps.
...
2. Filtering out interesting regions.  
```{r}
