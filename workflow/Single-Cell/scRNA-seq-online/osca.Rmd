...
counts <- left_join(x1, x2, by = c("GeneID" = "GeneID")) %>% 
    ...
    dplyr::filter(!grepl('^ERCC-|SIRV', GeneID)) 

mgs <- data.frame(ensembl = counts$GeneID)
mgs$symbol <- mapIds(org.Mm.eg.db, 
                     keys = mgs$ensembl, 
                     keytype = "ENSEMBL", 
                     column = "SYMBOL")

counts <- left_join(counts, mgs, by = c("GeneID" = "ensembl")) %>% 
    dplyr::select(-GeneID) %>% 
    dplyr::relocate(symbol, everything()) %>% 
