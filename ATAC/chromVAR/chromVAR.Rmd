...
## Peaks  
**For using chromVAR, it is recommended to use fixed-width, non-overlapping peaks**. The method is fairly robust to the exact choice of peak width, but a width of 250 to 500 bp is recommended. See the supplement of the paper for a disussion section and [supplementary](https://drive.google.com/file/d/0B8eUn6ZURmqvUjBCbE5Hc0p4UFU/view?usp=sharing) figures related to the choice of peak width.   
...
If combining multiple peak files from different populations, it is recommended to combine the peaks together. The `filterPeaks` function will reduce the peaks to a non-overlapping set based on which overlapping peak has stronger signal across all the data.  
...
peaks <- getPeaks(filename = peakfile, sort_peaks = TRUE)
```
...
fragment_counts <- getCounts(alignment_files = bamfile, 
                             peaks = peaks, 
                             paired = T, 
                             by_rg = T, 
                             format = "bam", 
...
example_counts <- addGCBias(object = example_counts, 
                            genome = BSgenome.Hsapiens.UCSC.hg19)

example_counts %>% rowData() %>% head()
```
...
```{r}
# find indices of sample to keep  
counts_filtered <- filterSamples(object = example_counts, 
                                 min_depth = 1500, 
                                 min_in_peaks = 0.15, 
                                 shiny = F)
```
...
filtering_plot <- filterSamplesPlot(object = example_counts, 
                                    min_depth = 1500, 
                                    min_in_peaks = 0.15, 
                                    use_plotly = F)
```
...
```{r}
# cells index that pass the criteria  
ix <- filterSamples(object = example_counts, min_depth = 1500, min_in_peaks = 0.15, shiny = F, ix_return = T)
...
```{r}
# like filter out low expressed genes in scRNA-seq
counts_filtered <- filterPeaks(object = counts_filtered, 
                               non_overlapping = T, 
...
```{r}
library(motifmatchr)
motif_ix <- matchMotifs(pwms = motifs, 
...
If instead of using known motifs, you want to use all kmers of a certain length, the `matchKmers` function can be used. For more about using kmers as inputs, see the [Annotations section of the documentation website](https://greenleaflab.github.io/chromVAR/articles/Articles/Counts.html).

```{r}
kmer_ix <- matchKmers(k = 6, 
                      subject = counts_filtered, 
                      genome = BSgenome.Hsapiens.UCSC.hg19)
```
...
To use the background peaks computed, simply add those to the call `computeDeviations()`.  
```{r}
dev <- computeDeviations(object = counts_filtered, 
                         annotations = motif_ix, 
                         background_peaks = bg)
...
```{r, warning=FALSE}
variablility <- computeVariability(object = dev)

plotVariability(variability = variablility, use_plotly = F, n = 5)
```
...
```{r}
tsne_results <- deviationsTsne(object = dev, 
                               threshold = 1.5, 
                               perplexity = 10)
```

To plot the results, `plotDeviationsTsne` can be used.  

```{r, warning=FALSE}
tsne_plots <- plotDeviationsTsne(object = dev, 
                                 tsne = tsne_results, 
