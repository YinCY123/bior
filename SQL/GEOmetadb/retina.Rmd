...
```{r}
dbReadTable(con, "sMatrix")
```

```{r}
dbReadTable(con, name = "gsm") %>%
    filter(organism_ch1 == "Homo sapiens") %>% 
    select(gsm, source_name_ch1, molecule_ch1) %>% 
    group_by(molecule_ch1) %>% 
    mutate(n = n()) %>% 
    distinct(source_name_ch1, molecule_ch1, .keep_all = T) %>% 
    filter(grepl("retina", source_name_ch1, ignore.case = T))
```
