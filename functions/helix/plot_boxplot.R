...
library(dplyr)
library(ggpubr)
plot_boxplot<-function(data=NULL,feature=NULL,group=NULL,colors=NULL,flip=F,feature_type=NULL,lab=NULL){
  #数据检测####
  ...
  boxplot_text=paste0("如图（Figure）所示，共有",sum(wilcoxtest$p.value<0.05&wilcoxtest$up_group==group_class[1]),
  ...
  #输出文字和表格####
  boxplot_test_list<<-list(boxplot_text=boxplot_text,
                           boxplot_wilcoxtest=wilcoxtest)
  #绘图####
