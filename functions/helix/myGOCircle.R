draw_table1 <- function(data, col){
  ...
  table <- tableGrob(subset(data, select = c(id, term,category)), cols = c('ID', 'Description','Category'), rows = NULL, theme = tt1)
  return(table)
}
                        ...
                        lfc.col, label.size, label.fontface) 
{
  ...
  if (is.numeric(nsub) == T) {
    ...
    if (strsplit(nsub[1], ":")[[1]][1] == "GO"|substr(nsub[1],1,3)%in% c("hsa","mmu")) {
  ...
  #调整点的颜色
  if(all(unique(cols)=="downregulated"))lfc.col.set=rev(lfc.col) else lfc.col.set=lfc.col
  ...
  c <- ggplot() + geom_rect(data = df, aes(xmin = x, xmax = x + 
    ...
    theme_blank + 
    scale_fill_gradient2("z-score",space = "Lab", low = zsc.col[3], mid = zsc.col[2], 
    ...
    scale_colour_manual(values = lfc.col.set,#调整颜色
  ...
  if (table.legend) {
    ...
    c=c + theme(plot.background = element_rect(fill = "white"), 
