pheatmap1<-function (mat, color = colorRampPalette(rev(brewer.pal(n = 7, 
                     ...
                     silent = FALSE, na_col = "#DDDDDD", name = NULL, heatmap_legend_param = list(), 
                     ..., run_draw = FALSE) 
{
  ...
  if (is.function(color)) {
    ...
    if (identical(breaks, NA)) {
      ...
      if (identical(scale, "row") || identical(scale, "column")) {
        ...
        ht_param$col = colorRamp2(seq(-lim, lim, length.out = n_col), 
                                  color)
      }
      else {
        ht_param$col = colorRamp2(seq(min(mat, na.rm = TRUE), 
                                      max(mat, na.rm = TRUE), length.out = n_col), 
                                  color)
      }
    }
    else {
  ...
  treeheight_row=treeheight_row*0.1*fontsize
  treeheight_col=treeheight_col*0.1*fontsize
  ...
  heatmap_legend_param = list(title_gp = gpar(fontsize = fontsize+1, fontface = "bold"),
                              grid_height = unit(fontsize*0.4, "mm"),grid_width = unit(fontsize*0.4, "mm"),
                              labels_gp = gpar(fontsize = fontsize),legend_direction = c("vertical"),
  ...
  if (!identical(annotation_col, NA)) {
      ...
      if (nm %in% names(annotation_colors)) {
        if (is.null(names(annotation_colors[[nm]])) && 
          ...
          annotation_colors[[nm]] = colorRamp2(seq(min(foo_x), 
                                                   max(foo_x), length.out = foo_n_col), annotation_colors[[nm]])
        }
      }
    }
    ht_param$top_annotation = HeatmapAnnotation(df = annotation_col[, 
                                                ...
                                                annotation_legend_param = list(title_gp = gpar(fontsize = fontsize+1, fontface = "bold"),
                                                                               grid_height = unit(fontsize*0.4, "mm"),grid_width = unit(fontsize*0.4, "mm"),
                                                                               labels_gp = gpar(fontsize = fontsize),legend_direction = c("vertical"),
                                                                               title_position = "topleft") ,
                                                gp = gpar(col = border_color), annotation_name_gp = gpar(fontsize = fontsize+1, 
                                                                                                         fontface = "bold"), simple_anno_size = unit(fontsize+1, 
...
#改变函数环境使得函数可用
environment(pheatmap1)=environment(ComplexHeatmap::pheatmap)
