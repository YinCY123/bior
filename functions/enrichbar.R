...
enrichbar <- function(res, top = 10, dir = NULL, text_size = 3, expand_L = 1, expand_R = 1, step = 2, width = 7, height = 7, ...){
    up <- res[, -"leadingEdge"] %>% as.data.frame %>% dplyr::filter(NES > 0) %>% dplyr::arrange(pval) %>% head(top)
    up$group <- "up"
    down <- res[, -"leadingEdge"] %>% as.data.frame %>% dplyr::filter(NES < 0) %>% dplyr::arrange(pval) %>% head(top)
    down$group <- "down"
